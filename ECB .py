...
def feistel(right, subkey):
    ...
    for i in range(8):
        ...
        val = S_BOX[i][row][col]
        bin_val = bin(val)[2:].rjust(4, '0')
        sbox_result.extend([int(b) for b in bin_val])
    # Permutation P
    p_result = permute(sbox_result, P)
    return p_result

def des_encrypt_block(block, subkeys):
    block_bits = string_to_bitlist(block)
    block_permuted = permute(block_bits, IP)
    L = block_permuted[:32]
    R = block_permuted[32:]
    for i in range(16):
        temp_R = R.copy()
        f_output = feistel(R, subkeys[i])
        R = [l ^ f for l, f in zip(L, f_output)]
        L = temp_R
    preoutput = R + L  # Note the swap
...
def ecb_decrypt(ciphertext, key):
    # Implementasi dekripsi mirip dengan enkripsi tetapi subkeys dibalik
    raise NotImplementedError("Dekripsi belum diimplementasikan.")

def main():
    # Contoh data dan kunci (harus 8 byte untuk DES)
    plaintext = b'This is a test message for DES ECB mode.'
    key = b'8bytekey'
